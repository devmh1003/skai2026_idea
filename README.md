...
| 모델 | 규모 | 컨텍스트 | 용도 |
|---|---|---|---|
| `skt/A.X-3.1-Light` (기본) | 7B | 32K | 사내 GPU 1장, 폐쇄망 |
| `skt/A.X-4.0-Light` | 7B | 16K | 최신 경량 |
| `skt/A.X-4.0` | 72B | 128K | 정밀 검토 |
| `skt/A.X-K2` | 692B MoE | — | API 전용, 최고 성능 |
...
| 백엔드 | 조건 | 비고 |
|---|---|---|
| `hf_api` | `.env`에 `HF_TOKEN` | GPU 불필요. 권장 |
| `local` | `torch` + `transformers` + GPU 16GB↑ | 폐쇄망·기밀 계약서용 |
| `offline` | 없음 | 룰 엔진만으로 코멘트. 네트워크 0회 |
| `auto` (기본) | — | local → hf_api → offline 순으로 자동 강등 |
...
| 탭 | 내용 |
|---|---|
| **개요** | 변경/위험 지표 카드, 위험도 도넛, 쟁점 카테고리 막대, 당사자별 유·불리 막대 |
| **조문 비교** | 좌우 대조 diff(삭제 취소선·추가 하이라이트), 위험도·구분·당사자·쟁점 필터, 전문 검색 |
| **당사자 영향** | 조문 × 당사자 매트릭스 — 다자간 계약에서 누가 무엇을 떠안는지 한 화면에 |
| **변경 이력** | 버전 체인 타임라인 (v1→v2→v3 구간별 수정/신설/삭제/고위험) |
