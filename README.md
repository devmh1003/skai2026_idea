# 체키 (Checky) — 계약서 비교 검토

두 버전의 계약서를 조문 단위로 대조하고, **SKT A.X 모델**(Hugging Face)로
조문마다 법무 코멘트를 생성하는 실무용 도구입니다.

화면에는 **체키**가 함께 다닙니다. 아래쪽을 천천히 오가다가 분석·업로드·저장이 시작되면
표정이 바뀌고(대기 / 검토 중 / 조항 발견 / 통과), 지금 무슨 일이 일어났는지 말해 줍니다.

- **입력**: 한글 `.hwpx` `.hwp` · Word `.docx` · `.pdf` · `.txt`
- **내보내기**: Word `.docx` · PDF(인쇄) · CSV · Markdown · JSON
- **처리**: 당사자 인식 → 조문 정렬 → 문장 단위 대비 → 검토 규정 기반 쟁점 탐지 → 당사자별 영향 추정 → 조문별 법무 코멘트
- **출력**: 단일 HTML 대시보드 · 마크다운 리포트 · JSON

> 생성 결과는 참고 자료이며 법률 자문이 아닙니다. 최종 판단은 변호사 검토를 거쳐야 합니다.

## 모델

기본값은 [`skt/A.X-3.1-Light`](https://huggingface.co/skt/A.X-3.1-Light) (7B · 32K 컨텍스트 · Apache-2.0)입니다.
한국어 계약 문언에 강하고, 32K 컨텍스트라 긴 조문도 잘리지 않습니다.

| 모델 | 규모 | 컨텍스트 | bf16 메모리 | 용도 |
|---|---|---|---|---|
| `skt/A.X-3.1-Light` (기본) | 7B | 32K | ~16GB | 사내 GPU 1장, 폐쇄망 |
| `skt/A.X-3.1` | 34B | 32K (YaRN 131K) | ~70GB | 정밀 검토 |
| `skt/A.X-4.0-Light` | 7B | 16K | ~16GB | 최신 경량 |
| `skt/A.X-4.0` | 72B | 128K | ~145GB | 장문 계약 일괄 |
| `skt/A.X-K2` | 692B MoE | — | — | API 전용 |

`--model` 또는 `.env`의 `CONTRACT_REVIEW_MODEL`로 교체합니다.

모델 연결만 따로 확인하려면:

```bash
python scripts/try_ax_model.py --backend local     # transformers 직접 로드
python scripts/try_ax_model.py --backend pipeline  # transformers.pipeline
python scripts/try_ax_model.py --backend hf_api    # GPU 불필요
```

## 설치

```bash
python -m venv .venv
.venv\Scripts\activate
pip install -e .
```

포맷별 선택 의존성:

```bash
pip install python-docx pypdf olefile   # docx / pdf / 구형 hwp
```

`.hwpx`와 `.txt`는 표준 라이브러리만으로 읽습니다.

## 백엔드

| 백엔드 | 조건 | 비고 |
|---|---|---|
| `adot_biz` | `ADOT_BIZ_BASE_URL` + `ADOT_BIZ_API_KEY` | SKT A.Biz 사내 게이트웨이. 계약서가 사외로 나가지 않음 |
| `hf_api` | `.env`에 `HF_TOKEN` | GPU 불필요 |
| `local` | `torch` + `transformers>=4.46` + GPU | `AutoModelForCausalLM`을 직접 로드 |
| `pipeline` | 위와 동일 | `transformers.pipeline("text-generation", ...)` 경로 |
| `offline` | 없음 | 룰 엔진만으로 코멘트. 네트워크 0회 |
| `auto` (기본) | — | adot_biz → local → hf_api → offline 순으로 자동 강등 |

`local` 백엔드는 모델 카드 권장 방식(`apply_chat_template(..., return_dict=True)` →
`generate(**inputs)`)을 그대로 따르며, `attention_mask`를 함께 넘깁니다.

```bash
cp .env.example .env   # HF_TOKEN 입력
```

## 사용법

### 파일 두 개 직접 비교

```bash
contract-review 원본.hwpx 개정본.docx --party all -o out
```

### 버전 관리로 운영 (권장)

협상은 보통 3~5회 왕복합니다. 버전 저장소에 등록해 두면 원본이 sha256으로 고정되고,
대시보드의 **변경 이력** 탭에 협상 경과가 그대로 쌓입니다.

```bash
contract-review version add 물류계약 당사초안.hwpx --label "당사 초안"
contract-review version add 물류계약 상대방1차.hwpx --label "상대방 1차"
contract-review version add 물류계약 상대방2차.hwpx --label "상대방 2차"

contract-review version list 물류계약
contract-review history 물류계약

contract-review review --contract 물류계약 --from v1 --to latest --party all
```

### 당사자 지정

당사자는 `(이하 "병"이라 한다)` 정의 문언에서 자동 인식하지만, 정의 문언이 없는 각서·합의서나
예시 문구를 잘못 잡는 경우가 있어 수동으로 보정할 수 있습니다.

```bash
contract-review review a.hwpx b.hwpx \
  --add-party "병=주식회사 사아자:연대보증인" \
  --remove-party 정
```

`--add-party`는 형식이 `약칭[=상호][:역할]`이고, 이미 인식된 당사자면 상호·역할만 덮어씁니다.
둘 다 반복 지정할 수 있습니다.

### A.Biz(에이닷 비즈) 연계

SK 그룹 전사에 도입된 업무용 AI 에이전트 [A.Biz](https://news.sktelecom.com/209598)와는
두 계층에서 붙습니다.

**① 모델 계층** — A.Biz 사내 LLM 게이트웨이로 A.X를 호출합니다. 계약서 원문이 사외로
나가지 않으므로 법무 검토 요건을 만족합니다. `.env`만 채우면 됩니다.

```bash
ADOT_BIZ_BASE_URL=https://<사내-게이트웨이>/v1
ADOT_BIZ_API_KEY=<발급키>
ADOT_BIZ_MODEL=ax-3.1-light
```

**② 에이전트 계층** — A.Biz의 Agent Builder에 '계약 검토' 에이전트를 만들고, 이 도구를 그
에이전트가 호출하는 사내 API로 등록합니다. 임직원은 대화창에 계약서 두 개를 올리고
"이전 버전과 뭐가 달라졌는지 봐줘"라고 쓰면, 에이전트가 이 파이프라인을 돌려 의견서 링크를
돌려줍니다. 버전 저장소가 계약 ID 단위로 이력을 쌓으므로 3차·4차 수정본이 와도 같은 대화에서
"지난번 대비 뭐가 또 바뀌었나"를 이어서 물을 수 있습니다.

게이트웨이가 OpenAI 호환이 아니면 [`llm/adot_biz.py`](src/contract_review_ai/llm/adot_biz.py)
한 파일만 교체하면 됩니다. 나머지 파이프라인은 백엔드 종류를 알지 못합니다.

### 주요 옵션

| 옵션 | 설명 |
|---|---|
| `--party 을` / `--party all` | 특정 당사자 관점 또는 전 당사자 관점으로 코멘트 생성 |
| `--add-party` / `--remove-party` | 당사자 수동 추가·제외 |
| `--min-level medium` | 중위험 이상 조문만 LLM 호출 (비용·시간 절감) |
| `--backend hf_api` | 백엔드 강제 지정 |
| `--format html,md,json` | 출력 형식 선택 |
| `-o out` | 출력 폴더 |

고위험 변경이 하나라도 있으면 종료코드 `1`을 반환하므로, 결재 파이프라인의 게이트로 쓸 수 있습니다.

### 데모

```bash
python scripts/run_demo.py
```

3자 계약(갑·을·병) 샘플로 v1·v2를 등록하고 `out/dashboard.html`을 만듭니다. 네트워크를 쓰지 않습니다.

SK 계열사 상황을 가정한 예시 계약 10종을 만들려면(모두 가상 문서입니다):

```bash
python scripts/make_samples.py --reset
```

분류는 **물품구매 · 용역 · 자문 · 연구개발 · 라이선스** 5종이며, 계약마다 협상이 멈춘 지점이
달라 현황 화면에 개시·진행중·완료가 섞여 나타납니다.

### 쟁점 룰 커스터마이즈

내장 룰 23종은 어느 계약에나 걸리는 공통 쟁점만 담았습니다. 업종·회사마다 반드시 봐야 하는
항목은 파일로 얹고, 필요 없는 룰은 끌 수 있습니다.

```bash
contract-review rules list                     # 적용 중인 룰 확인
contract-review rules export my_rules.json     # 편집 시작점 내보내기
contract-review review a.hwpx b.hwpx \
  --rules my_rules.json --disable-rule MFN --disable-rule NUMERIC-CHANGE
```

룰 파일은 JSON 또는 TOML이며, 같은 `code`를 쓰면 내장 룰을 덮어씁니다(문구를 사내 표현으로
바꾸거나 위험도만 조정할 때). 예시는 [`data/rules/custom_rules.example.json`](data/rules/custom_rules.example.json)
— 하도급대금 직접지급, 개인정보 국외이전, ESG 공급망 실사, AI 학습 데이터 이용 4종이 들어 있습니다.

```json
{
  "rules": [
    {
      "code": "PRIVACY-CROSSBORDER",
      "category": "개인정보",
      "level": "high",
      "mode": "introduced",
      "pattern": "국외\\s*(이전|제공|저장)|해외\\s*서버",
      "message": "개인정보 국외 이전 문언이 추가되었습니다. …"
    }
  ],
  "disable": ["MFN"]
}
```

`mode`는 `introduced`(개정본에서 새로 등장) · `removed`(원본에 있었는데 사라짐) ·
`changed`(문언이 바뀌었고 어느 쪽이든 걸림) 중 하나입니다.

### 계약 생성·첨부

```bash
contract-review new 2026-용역-007 초안.hwpx 상대방1차.hwpx   --title "AI 플랫폼 구축 용역" --category "용역·도급"   --label "당사 초안" --label "상대방 1차"

contract-review attach 2026-용역-007 상대방2차.docx --label "상대방 2차" --note "IP 조항 재수정"
```

`new`는 계약을 만들고 파일을 v1, v2… 순서로 첨부합니다. 파일 없이 계약 껍데기만 만들 수도
있습니다. `attach`는 기존 계약에 이어 붙입니다. 내용이 같은 파일은 중복 등록되지 않습니다.

### 내보내기

`--format`으로 조합하며 `all`이면 전부 만듭니다.

| 형식 | 용도 |
|---|---|
| `html` | 검토 리포트 (단일 파일) |
| `md` | 메일·이슈트래커 붙여넣기 |
| `json` | 후속 시스템 연계 |
| `csv` | **엑셀 협상 대응표** — 조문 1건 = 1행, 관점별 코멘트까지 펼침 |

파일명은 계약 ID와 버전으로 자동 결정됩니다 — `2026-용역-007_v1-v3.csv`.
버전 저장소를 쓰지 않으면 문서명 기준으로 떨어집니다.

워크스페이스에서는 `--export csv`로 **계약대장·버전대장·조문표**를 한 번에 만듭니다.

```bash
contract-review workspace --party all --export csv -o out
# out/계약대장.csv, out/버전대장.csv, out/조문표/*.csv, out/workspace.html
```

### 문서 보호 — 암호화와 등록 원장

```bash
export CONTRACT_REVIEW_KEY="사내-열쇠-문구"
contract-review serve
```

열쇠가 설정되면 등록되는 원본을 **ChaCha20(RFC 8439) + HMAC-SHA256**으로 암호화해
`*.enc`로 저장합니다. 저장소 폴더를 통째로 복사해도 평문이 나오지 않습니다.
해시는 평문 기준으로 기록하므로 암호화 여부와 무관하게 같은 문서는 같은 sha256을 갖습니다.

등록·편집이 일어날 때마다 **블록체인(`data/ledger.jsonl`)에 블록이 하나** 붙습니다.
각 블록은 앞 블록의 해시를 품고 있어, 중간 기록을 고치면 뒤 블록의 해시가 전부 어긋납니다.

블록·해시·무결성은 전부 **Blockchain** 탭 한 곳에 모여 있습니다. 다른 화면은 계약 업무만
다루고, 기록의 무결성은 이 탭에서 봅니다.

이 탭은 6초마다 서버에 상태를 물어 갱신합니다 — 블록 높이·체인 팁·전수 검증 소요 시간·
원장 크기·평균 블록 간격이 모두 그때그때 실제로 잰 값입니다. **체인 지도**는 블록을 접어
늘어놓고 연결 순서대로 선을 이어, 하나가 앞을 물고 있는 구조를 그대로 보여 줍니다.
업로드나 조문 편집으로 블록이 늘면 새로고침 없이 지도에 칸이 하나 더 붙습니다.

```bash
contract-review ledger verify        # 체인 검증 (불일치 시 종료코드 1)
contract-review ledger list 2026-자문-003
```

분산 합의는 하지 않습니다 — 노드도 채굴도 없습니다. 한 조직 안에서 '기록을 조용히 고칠 수
없게' 만드는 것이 목적이고 거기에는 해시 체인으로 충분합니다. 외부에 증명해야 한다면
최신 블록 해시(체인 팁)를 주기적으로 타임스탬프 기관에 고정하면 됩니다.

암호 구현의 한계는 [`crypto.py`](src/contract_review_ai/crypto.py) 첫머리에 적어 두었습니다 —
검증된 암호 라이브러리의 대체물이 아니고, 열쇠를 가진 사람으로부터 지켜 주지는 않습니다.

### 계약 관계망

계약은 한 건씩 떨어져 있는 것처럼 다루기 쉽지만 실제로는 얽혀 있습니다. 같은 상대방과
여러 건을 맺고 있으면 한 곳에서 밀린 조건이 다른 계약으로 번지고, 같은 쟁점이 여러 계약에
흩어져 있으면 한 번에 정리하는 편이 낫습니다. 현황관리의 관계망이 그 얽힘을 보여줍니다.

- **노드** = 계약. 크기는 버전 수, 색은 분류
- **실선** = 같은 조직이 당사자로 들어간 계약 · **점선** = 쟁점이 셋 이상 겹치는 계약
- 노드에 올리면 그 계약과 이어진 것만 남고, 누르면 해당 계약으로 이동합니다

배치는 서버에서 스프링 모형(Fruchterman-Reingold)으로 계산해 좌표를 박아 넣습니다.
브라우저에서 물리를 돌리지 않으므로 열 때마다 그림이 흔들리지 않고, 외부 그래프
라이브러리도 필요 없습니다.

### 계약 기한

계약기간 조문에서 **종료일·갱신 거절 통지 기한·자동 갱신 여부**를 읽어 현황관리에 올립니다.
자동 갱신 계약은 통지 기한을 넘기면 조건이 그대로 한 해 더 가므로, 통지 기한이 있으면
그쪽을 기준으로 남은 날짜를 셉니다. `체결일부터 2년`처럼 기산점이 문서 밖에 있으면
값을 지어내지 않고 비워 둡니다.

### 서버 모드 — 브라우저에서 올리고 내려받기

```bash
contract-review serve --party all
# http://127.0.0.1:8000
```

표준 라이브러리만으로 도는 로컬 서버입니다(외부 패키지·인터넷 불필요). 브라우저에서

- **계약 등록 / 버전 추가** — 한글·Word·PDF 원본을 올리면 그 자리에서 버전으로 등록되고
  검토가 다시 계산됩니다. 여러 파일을 한 번에 올리면 v1, v2… 순서로 붙습니다.
- **당사자 지정** — 등록할 때 `갑=주식회사 가나다:발주자` 형식으로 지정하면 이후 모든
  비교본에 적용됩니다. `-병` 처럼 앞에 -를 붙이면 자동 인식된 당사자를 제외합니다.
- **표준 양식 받기** — 초안이 없으면 표준 계약서 5종을 Word로 받아 수정한 뒤 그대로 올립니다.
- **내려받기** — 비교본마다 Word(.docx)와 PDF(브라우저 인쇄), 대시보드에서 계약대장·버전대장.
  파일명은 계약 ID와 버전으로 자동 결정됩니다.
- **A.Biz 회의록 불러오기** — 회의 반영 패널에서 `회의록 불러오기`를 누르면 그 계약에
  연결된 회의 목록이 뜨고, 고르면 회의 내용이 채워지며 조문 반영안까지 자동으로 만듭니다.
  현재는 **실제 A.Biz를 호출하지 않는 데모 데이터**이며(화면에도 그렇게 표시됩니다),
  회의록은 해당 계약의 실제 조문에서 만들어져 반영 결과는 진짜 조문에 걸립니다.
  실제 연동 시 `serve.py`의 `_meetings` 한 곳만 A.Biz 호출로 바꾸면 됩니다.
- **회의 반영** — 협상 회의록을 붙여 넣으면 합의 사항이 어느 조문에 걸리는지 찾아
  수정 문안을 제안합니다. 조 번호를 적었으면(`제3조 …`) 그 조문에, 아니면 문언 유사도로
  가장 가까운 조문에 붙입니다. 계약과 무관한 항목(다음 회의 일정 등)은 따로 분리해 알려 줍니다.
  채택한 조문만 모아 새 버전으로 저장합니다.
- **버전 원문 보기 · 조문 편집** — 버전 관리 표에서 버전을 선택하면 그 시점의 조문 전문이
  펼쳐지고, `조문 편집`으로 문안을 직접 고쳐 **새 버전으로 저장**할 수 있습니다. 등록된 원본은
  해시로 고정돼 있어 덮어쓰지 않습니다 — 무엇을 고쳐 보냈는지가 이력에 그대로 남습니다.

파일을 그냥 열었을 때(`file://`)는 저장할 곳이 없으므로 업로드·내려받기 버튼이 같은 작업을
수행하는 CLI 명령을 안내합니다 — 되는 척하지 않습니다.

### 워크스페이스 — 계약·버전·검토를 한 화면에서

```bash
contract-review workspace --party all -o out
```

등록된 모든 계약의 버전 조합을 검토해 `out/workspace.html` 하나로 묶습니다. 서버 없이 열리는
단일 파일이지만 화면 구성은 콘솔에 가깝습니다.

| 화면 | 내용 |
|---|---|
| **현황관리** | 진행 단계(개시·진행중·완료), **계약 관계망**, 문서 원장, 다가오는 기한, 지표, 최근 개정 활동 |
| **계약상세** | 분류 필터 + 검색이 붙은 계약 목록 → 버전 대장(주요 변경·SHA-256), 개정 타임라인, 쟁점 분포, 비교본 목록 → 검토 결과 |
| **계약생성** | 계약 등록·당사자 지정·표준 양식 내려받기 |
| **고객관리** | 계약별 당사자 명부 |
| **원장** | 등록·편집 블록 체인. 각 블록이 앞 블록의 해시를 품어 기록을 조용히 고칠 수 없습니다 |
| **조항검색** | 모든 계약의 조문을 가로질러 문구로 찾기. 여러 단어를 띄어 쓰면 모두 포함된 조문을 찾고, 협상 중 삭제된 문언은 `이전 버전 포함`으로 찾습니다 |

**당사자 관리**는 화면에서 직접 합니다. 약칭·상호·역할을 고치면 검토 결과의 영향 매트릭스와
조문 배지에 즉시 반영되고, 삭제하면 목록과 결과에서 모두 사라집니다(되돌리기 가능). 편집은
브라우저에 저장돼 다시 열어도 유지됩니다. 당사자를 새로 추가하면 권리·의무 점수를 다시
계산해야 하므로 재실행 명령을 함께 띄웁니다.

조문 대비는 **문장 단위가 기본**이며, 필요하면 단어 단위로 전환합니다. 좌우 대비와 통합 대조도
전환할 수 있습니다. `--pairs adjacent|all|latest`로 비교 조합을 조절합니다.

## 검토 의견서 (HTML)

외부 CDN·폰트·스크립트 의존이 전혀 없는 단일 HTML입니다(폐쇄망 반입 가능). 차트는 파이썬이 SVG로 직접 그립니다.
엔터프라이즈 도구 톤 — 중립 무채색 표면에 액센트 하나, 얇은 보더, 낮은 라운드, 표 형태 수치 —
으로 정리했고, 인쇄용 스타일(`@media print`)이 있어 탭 없이 전체가 한 문서로 출력됩니다.

지표 요약(변경·수정·신설·삭제·고위험·중위험 + 위험도 막대)은 탭 위 한 줄로 고정됩니다.

| 탭 | 내용 |
|---|---|
| **조문 대비** | 조문마다 **변경 요지**(삭제↔추가 대응표)를 먼저 보여주고, 좌우 대비 / 통합 대조를 전환할 수 있습니다. 위험도·구분·당사자·쟁점 필터와 전문 검색 제공 |
| **당사자 영향** | 당사자별 유·불리 막대 + 조문 × 당사자 매트릭스 — 다자간 계약에서 누가 무엇을 떠안는지 한 화면에 |
| **개정 연혁** | 버전 체인 타임라인 (v1→v2→v3 구간별 수정/신설/삭제/고위험) |

## 다자간 계약

`(이하 "갑"이라 한다)` 형태의 정의 문언을 파싱해 당사자를 자동 인식합니다. 갑·을뿐 아니라
병·정, 그리고 `(이하 "수급인"이라 한다)` 같은 역할 약칭도 잡습니다.

각 조문에서 당사자별로 **권리 표현**(…할 수 있다, 청구할 수 있다)과 **의무 표현**
(…하여야 한다, 부담한다, 배상한다)을 문장 단위로 세어 `권리 − 의무` 점수를 내고,
변경 전후 점수 차이로 유리/불리 방향을 추정합니다. 법적 판단이 아니라 **어디를 먼저 볼지
정하는 신호**이며, 리포트에도 그렇게 표기됩니다.

`--party all`을 주면 당사자 수만큼 관점별 코멘트를 각각 생성합니다.

## 위험 룰

LLM에 판단을 통째로 맡기지 않고, 결정론적 룰로 먼저 쟁점을 좁힙니다. 룰 결과는
(1) 프롬프트의 근거로 들어가고 (2) 모델이 놓치더라도 리포트에 그대로 남습니다.

손해배상(무제한 배상·책임 한도 삭제·간접손해 배제 삭제), 위약벌, 계약해지(일방적 해지·
시정기간 삭제), 지식재산권(귀속 변경·저작인격권 불행사), 면책·보상, 지체상금, 대금지급
(기일 연장·일방적 감액), 비밀유지, 준거법·분쟁해결, 자동갱신, 재위탁, 권리양도, 경업금지,
개인정보, 불가항력, 하자담보, 최혜대우, **연대보증(포괄근보증)**, 수치 변경(기간·요율·금액)
— 총 23종.

## 구조

```
contract-review-ai/
├── src/contract_review_ai/
│   ├── parsing/       hwpx·hwp·docx·pdf 로더, 조(條) 단위 분할
│   ├── parties/       당사자 인식, 당사자별 유·불리 영향 추정
│   ├── diffing/       문자 3-gram 유사도, 조문 정렬, 단어 diff
│   ├── risk/          룰 엔진 (23종)
│   ├── llm/           백엔드 (local A.X / HF API / offline) + JSON 파싱
│   ├── review/        프롬프트, 파이프라인 오케스트레이션
│   ├── versioning/    버전 저장소(sha256 고정), 변경 이력 타임라인
│   ├── report/        HTML 대시보드, 마크다운
│   └── cli.py         review / version / history 커맨드
├── data/samples/      3자 계약 샘플 (v1, v2)
├── scripts/run_demo.py
└── tests/
```

## 설계 원칙

- **파싱·정렬·diff·룰은 100% 결정론적** — 같은 입력이면 언제 돌려도 같은 결과. LLM은 코멘트 문장만 담당합니다.
- **코멘트마다 출처를 표기** — `offline(룰기반)`인지 `hf_api:skt/A.X-3.1-Light`인지 리포트에 남습니다.
- **모델 호출 실패는 조문 단위로 격리** — 한 조문이 실패해도 나머지 검토는 끝까지 진행됩니다.
- **위험도는 보수적으로** — 룰 판정과 LLM 판정 중 높은 쪽을 채택합니다.

## 테스트

```bash
pytest -q
```
