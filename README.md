...
- **계약 등록 / 버전 추가** — 계약서를 끌어다 올리면 그 자리에서 버전으로 등록되고
  검토가 다시 계산됩니다. 여러 파일을 한 번에 올리면 v1, v2… 순서로 붙습니다.
- **내려받기** — 비교본마다 CSV·Markdown·HTML·JSON을, 대시보드에서 계약대장·버전대장 CSV를
  즉석 생성해 받습니다. 파일명은 계약 ID와 버전으로 자동 결정됩니다.
...
| 화면 | 내용 |
|---|---|
| **대시보드** | 관리 계약·등록 버전·누적 변경·고위험 지표, 분류별 분포, 최근 개정 활동 |
| **계약** | 분류 필터 + 검색이 붙은 계약 목록. 행을 선택하면 상세로 |
| **계약 상세** | 버전 대장(등록일·SHA-256·메모), **당사자 관리**, 개정 타임라인, 쟁점 분포, 비교본 목록 |
| **검토 결과** | 조문 대비 / 당사자 영향 / 개정 연혁 |
